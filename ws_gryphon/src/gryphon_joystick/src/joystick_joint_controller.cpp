...
public:
    JoystickJointController() : Node("joystick_joint_controller"), current_axes_(8, 0.0), current_joint_positions_(6, 0.0)
    {
        ...
        joint_task_client_ = rclcpp_action::create_client<gryphon_server::action::JointTask>(this, "joint_task");
...
private:
    ...
    rclcpp_action::Client<gryphon_server::action::JointTask>::SharedPtr joint_task_client_;
    ...
    void sendGoalIfNeeded(){
        ...
        current_joint_positions_[5] += (current_axes_[2] - 1) * step;
        current_joint_positions_[5] -= (current_axes_[5] - 1) * step;

        // Create a goal for JointTask
        auto goal_msg = gryphon_server::action::JointTask::Goal();
        ...
        goal_msg.gripper_joint_deg = current_joint_positions_[5];

        // Print
        RCLCPP_INFO(this->get_logger(), "Sending JointTask goal: %f, %f, %f, %f, %f, %f", goal_msg.joint1_deg, goal_msg.joint2_deg, goal_msg.joint3_deg, goal_msg.joint4_deg, goal_msg.joint5_deg, goal_msg.gripper_joint_deg);

        // Send the goal to the action server
        auto send_goal_options = rclcpp_action::Client<gryphon_server::action::JointTask>::SendGoalOptions();
        send_goal_options.result_callback = [this](const rclcpp_action::ClientGoalHandle<gryphon_server::action::JointTask>::WrappedResult &result) {
