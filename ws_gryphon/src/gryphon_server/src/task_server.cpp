...
#include "gryphon_server/action/joint_task.hpp"
#include "gryphon_server/action/pose_task.hpp"
...
public:
  explicit TaskServer(const rclcpp::NodeOptions& options = rclcpp::NodeOptions()) : Node("task_server", options)
  {
    ...
    // JointTask Server
    joint_task_server_ = rclcpp_action::create_server<gryphon_server::action::JointTask>(
        this, "joint_task",
    ...
    // PoseTask Server
    pose_task_server_ = rclcpp_action::create_server<gryphon_server::action::PoseTask>(
        this, "pose_task",
...
private:
  ...
  // Action servers
  rclcpp_action::Server<gryphon_server::action::JointTask>::SharedPtr joint_task_server_;
  rclcpp_action::Server<gryphon_server::action::PoseTask>::SharedPtr pose_task_server_;

  // JointTask callbacks
  rclcpp_action::GoalResponse jointTaskGoalCallback(const rclcpp_action::GoalUUID& uuid, std::shared_ptr<const gryphon_server::action::JointTask::Goal> goal)
  {
  ...
  rclcpp_action::CancelResponse jointTaskCancelCallback(const std::shared_ptr<rclcpp_action::ServerGoalHandle<gryphon_server::action::JointTask>> goal_handle)
  {
  ...
  void jointTaskAcceptedCallback(const std::shared_ptr<rclcpp_action::ServerGoalHandle<gryphon_server::action::JointTask>> goal_handle)
  {
  ...
  void jointTaskExecute(const std::shared_ptr<rclcpp_action::ServerGoalHandle<gryphon_server::action::JointTask>> goal_handle){
    ...
    auto result = std::make_shared<gryphon_server::action::JointTask::Result>();

    arm_joint_goal_ = {goal_handle->get_goal()->joint1_deg, goal_handle->get_goal()->joint2_deg, goal_handle->get_goal()->joint3_deg, goal_handle->get_goal()->joint4_deg, goal_handle->get_goal()->joint5_deg};
    ...
    gripper_joint_goal_ = {gripper_joint_rad, -gripper_joint_rad};
  ...
  // PoseTask callbacks
  rclcpp_action::GoalResponse poseTaskGoalCallback(const rclcpp_action::GoalUUID& uuid, std::shared_ptr<const gryphon_server::action::PoseTask::Goal> goal)
  {
  ...
  rclcpp_action::CancelResponse poseTaskCancelCallback(const std::shared_ptr<rclcpp_action::ServerGoalHandle<gryphon_server::action::PoseTask>> goal_handle)
  {
    RCLCPP_INFO(get_logger(), "Received request to cancel PoseTask");
    return rclcpp_action::CancelResponse::ACCEPT;
  }

  void poseTaskAcceptedCallback(const std::shared_ptr<rclcpp_action::ServerGoalHandle<gryphon_server::action::PoseTask>> goal_handle)
  {
  ...
  void poseTaskExecute(const std::shared_ptr<rclcpp_action::ServerGoalHandle<gryphon_server::action::PoseTask>> goal_handle){
    RCLCPP_INFO(get_logger(), "Executing goal");
    if(!arm_move_group_){
      arm_move_group_ = std::make_shared<moveit::planning_interface::MoveGroupInterface>(shared_from_this(), "arm_group");
    }

    auto result = std::make_shared<gryphon_server::action::PoseTask::Result>();
...
RCLCPP_COMPONENTS_REGISTER_NODE(gryphon_server::TaskServer) 